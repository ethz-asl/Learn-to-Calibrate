...
def stack_samples(samples):
    ...
    current_states = np.stack(array[:,0]).reshape((-1,array[0,0].shape[0]))
    current_act_hists = np.stack(array[:,1]).reshape((-1,array[0,1].shape[0]))
    actions = np.stack(array[:,2]).reshape((-1,array[0,2].shape[0]))
    rewards = np.stack(array[:,3]).reshape((-1,1))
    new_states = np.stack(array[:,4]).reshape((-1,array[0,4].shape[0]))
    new_act_hists = np.stack(array[:,5]).reshape((-1,array[0,5].shape[0]))
    dones = np.stack(array[:,6]).reshape((-1,1))
...
class ActorCritic:
    def __init__(self, sess):
        ...
        self.critic_learning_rate = 1e-5  #1e-4 #5e-5 #5e-5
        self.actor_learning_rate = 1e-6   #1e-4 #5e-5 #5e-5
        self.batch_size = 64
    ...
    def create_actor_model(self):
        ...
        #actor_rnn,state_h,state_h_1 =LSTM(64, return_state=True)(Concatenate()([state_input,act_hist_input]))
        #state_h = Dense(64, return_state=True)(Concatenate()([obs_h1,act_h1]))
        h2 = Dense(256, activation='relu')(Concatenate()([state_input,act_input]))
        output = Dense(self.act_dim, activation='tanh')(h2)

        model = Model([state_input,act_input], output)
        adam  = Adam(lr=0.0001)
        model.compile(loss="mse", optimizer=adam)
        return state_input, act_input, model
    ...
    def create_critic_model(self):
        ...
        # critic_rnn,state_h2,state_c2 = LSTM(64, return_state=True,kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(Concatenate()([state_input,act_input]))

        action_input = Input(shape=self.act_dim)
        action_h1    = Dense(256,kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(action_input)

        merged    = Concatenate()([state_input,act_input, action_h1])
        ...
        model  = Model([state_input,act_input,action_input],output)

        adam  = Adam(lr=self.critic_learning_rate)
        model.compile(loss="mse", optimizer=adam)
        return state_input, act_input, action_input, model
...
def read_replay(path,actor,obs_dim,action_dim):
    ...
    for i in range(len(obs)):
        ...
        next_single_obs = next_obs_seq[-1,:]
        next_act_seq = next_obs_action_seq[:,12:48]
        next_single_act = next_act_seq[-1,:]
        ...
        cur_obs = cur_obs.reshape((obs_dim))
        cur_act = cur_act.reshape((action_dim))
        action = action.reshape((action_dim))
        next_single_obs = next_single_obs.reshape((obs_dim))
        next_single_act = next_single_act.reshape((action_dim))
        print(cur_obs)
        print(cur_act)
        print(action)
        print(reward)
        ...
        actor.remember(cur_obs, cur_act, action, reward, next_single_obs, next_single_act, done)
        #augment data
