...
class ActorCritic:
    def __init__(self,sess):
        ...
        self.trans_learning_rate = 1e-3  #1e-4 #5e-5 #5e-5
        self.reward_learning_rate = 1e-3   #1e-4 #5e-5 #5e-5
...
def PSO_grad_optimize(n_particles, dimensions,  bounds, init_pose, select_num, iteration, lr, init_state, init_act):
    ...
    step_num = int(dimensions/action_dim)
    scales = np.tile(scale,step_num)
    ...
    for i in range(iteration):
        ...
        my_swarm.pbest_pos = np.clip(my_swarm.pbest_pos,-scales,scales)
        ...
        my_swarm.position = np.clip(my_swarm.position,-scales,scales)
    ...
    return np.clip(my_swarm.best_pos,-scales,scales), np.asarray(good_actions)
...
def act(step_num, israndom, isdeter):
    ...
    #random action
    if israndom:
        return np.random.uniform(-scale,scale,actor_critic.act_dim)
    ...
    bound = np.multiply(np.ones(dimensions),scales)
    bounds = (-bound, bound)

    #substitute part of the init value to be the last global optimum
    init_pose = np.random.uniform(-scales,scales,(15,dimensions))
    global_opt = []
    for i in range(trial_len):

        if step_num==i+1:
            global_opt = copy.copy(global_opt_list[trial_len-step_num])
    ...
    action, good_actions = PSO_grad_optimize(n_particles=15,dimensions=dimensions,
                                             bounds=bounds,init_pose=init_pose,select_num=select_number,
    ...
    if isdeter:
        ...
        sel_ind = np.random.choice(np.arange(0,select_number,1),1)
    ...
    for i in range(trial_len):
        if step_num == i+1:
            global_opt_list[trial_len-step_num] = good_actions[:save_number]
...
def step(action, last):
    ...
    try:
        ...
        return np.zeros(obs_dim), 0
    return resp1.next_state, resp1.reward
...
def simulation():
    ...
    for i in range(num_trials):
        ...
        for j in range(trial_len):
            ...
            new_state, reward = step(action,last)
            reward_sum += reward
