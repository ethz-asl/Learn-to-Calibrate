...
class EncoderNetwork(nn.Module):
    ...
    def forward(self, set_state, nom_state):
        set_x = F.relu(self.bn1(self.conv1(set_state)))
        ...
        set_x_value = F.relu(self.bn2(self.conv2(set_x)))
        ...
        set_x_value = F.relu(self.bn3(self.conv3(set_x)))
...
class AgentNetwork(nn.Module):
    def __init__(self, set_obs_dim, nom_obs_dim, num_inputs, num_actions, hidden_size, init_w=3e-3, log_std_min=-20, log_std_max=2):
        ...
        self.encoder_net = EncoderNetwork(set_obs_dim, nom_obs_dim, num_inputs)
    ...
    def forward(self, set_state, nom_state, action, next_set_state, next_nom_state):
        state = self.encoder_net(set_state, nom_state).detach()
        next_state = self.encoder_net(next_set_state, next_nom_state)
        expected_q_value = self.soft_q_net(state, action)
        expected_value   = self.value_net(state)
        new_action, log_prob, z, mean, log_std = self.evaluate(state)
        expected_new_q_value = self.soft_q_net(state, new_action)

        return state, next_state, expected_q_value, expected_value, new_action, log_prob, z, mean, log_std, expected_new_q_value
    ...
    def get_action(self, set_state, nom_state):
        set_state = torch.FloatTensor(set_state).unsqueeze(0).to(device)
        nom_state = torch.FloatTensor(nom_state).unsqueeze(0).to(device)
        state = self.encoder_net(set_state, nom_state)
        mean, log_std = self.policy_net(state)
        std = log_std.exp()
...
def soft_q_update(batch_size, 
    ...
    state, next_state, expected_q_value, expected_value, new_action, log_prob, z, mean, log_std, expected_new_q_value = agent_net(set_state, nom_state, action, next_set_state, next_nom_state)
    ...
    policy_loss += mean_loss + std_loss + z_loss + sr * state.pow(2).mean()
...
value_optimizer  = optim.Adam(list(agent_net.value_net.parameters()) + list(agent_net.encoder_net.parameters()), lr=value_lr)
soft_q_optimizer = optim.Adam(list(agent_net.soft_q_net.parameters()) +  list(agent_net.encoder_net.parameters()), lr=soft_q_lr)
policy_optimizer = optim.Adam(list(agent_net.policy_net.parameters()) +  list(agent_net.encoder_net.parameters()), lr=policy_lr)
