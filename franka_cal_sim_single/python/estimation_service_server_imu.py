...
from franka_cal_sim_single.srv import recordSrv, estimateSrv, recordSrvResponse, estimateSrvResponse
...
# #kalibr command
kalibr_path = "/home/yunke/prl_proj/panda_ws/src/franka_cal_sim_single/python/kalibr"
k_command = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 10 --dont-show-report"
k_command_total = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 10 --dont-show-report"
...
def spawn_model(spawn_flag, restart):
    ...
    if spawn_flag:
        ...
        sample_k = np.random.normal(centor_k,0.1)
        centor_center = np.asarray([0.5,0.5])
        sample_center = np.random.normal(centor_center,0.05)
        ...
        '''broacast new params'''
        rospy.set_param("/rl_client/imu_cam_ground_truth",sample_ext.astype(float).tolist())
...
def estimate_callback(req):
    ...
    #total_step = rospy.get_param("/rl_client/num_steps")
    ...
    if not req.reset == 1:
        ...
        if req.done:
            #first calibrate intrinsic
            best_mtx, ipts, opts, progress, state_progress, reproj_err = camera_intrinsic_calibration2(req, image_data)
            # change the yaml file of camera
            ...
            res.cal_err = res.cal_err/np.sqrt(np.sum(np.square(extrinsic[0:3,0:3]))+np.sum(np.square(ground_truth[0:3])))
            rospy.loginfo(rospy.get_caller_id() + 'I get ground truth %s', ground_truth_mat)
            rospy.loginfo(rospy.get_caller_id() + 'I get ground truth pos%s', position)
        ...
        if (num_step >=cal_num_step or req.done) and if_change_model:
        ...
        res.empirical = (len(ipts) - obs_n)/float(max([len(image_data),1.0]))*10
        obs_n = len(ipts)

        #compute return items
        
        res.empirical += (orientation_x_en+orientation_w_en+orientation_y_en+orientation_z_en)*0.1
        rospy.loginfo(rospy.get_caller_id() + 'I get cal_err %s', res.cal_err)
        rospy.loginfo(rospy.get_caller_id() + 'I get ground truth %s', ground_truth)
