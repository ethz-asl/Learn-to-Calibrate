...
while episode < 10: 
    ...
    reset(0)
    reset(0)
    state = np.concatenate([np.ones((12,))*0.5, np.zeros((6,))])
    #init size
    state[8] = 0.28
    state[9] = 0.28
    episode_reward = 0
    ...
    '''intrinsic (4), distortion (3), dist_center(2), intrinsic accuracy, coverage, path length, episode length, rewards'''
    
    #intrinsic, extrinsic
    intrinsic = rospy.get_param('rl_client/cam_ground_truth') 
    ...
    for j in range(max_steps):
        ...
        if done or j==max_steps-1:
            ...
            data = intrinsic + distortion_K + dist_center
            data.append(cal_err)
            data.append(coverage)
            data.append(path_len)
            data.append(episode_len)
...
np.savetxt("experiment data",data_array)
