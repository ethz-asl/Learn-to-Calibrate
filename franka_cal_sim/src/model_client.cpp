...
bool model_client::step(franka_cal_sim::RLSrv::Request &req, franka_cal_sim::RLSrv::Response &res)
{
  ...
  else
  {
    ...
    {
      // distance from ground truth last step
    ...
    {
      // distance from ground truth this step
      post_error += pow(res.next_state[i] - ground_truth[i], 2);
    }
    post_error = sqrt(post_error);
    // error change in this step
    double err_change = post_error - pre_error;

    // I think the items in rewards should be weighted or normalized.
    res.reward =
        estimate_srv.response.obs + estimate_srv.response.coverage - err_change - action_srv.response.path_len * 0.1;
