...
namespace franka_cal_sim
{
	...
	bool simulation_server::model(franka_cal_sim::actionSrv::Request &req,franka_cal_sim::actionSrv::Response &res)
	{
		...
		for(int i=0;i<num_of_step;i++)
		{
		   ...
		   pose.position.z += action[16]*cos(4*theta)+action[17]*sin(4*theta)-(action[12]+action[14]+action[16])+0.4;
		   R = action[18]*cos(theta)+action[19]*sin(theta)+action[20]*cos(2*theta)+action[21]*sin(2*theta);
		   R += action[22]*cos(4*theta)+action[23]*sin(4*theta)-(action[18]+action[20]+action[22])+1.57;
		   P = action[24]*cos(theta)+action[25]*sin(theta)+action[26]*cos(2*theta)+action[27]*sin(2*theta);
		   P += action[28]*cos(4*theta)+action[29]*sin(4*theta)-(action[24]+action[26]+action[28])+1;
		   Y = action[30]*cos(theta)+action[31]*sin(theta)+action[32]*cos(2*theta)+action[33]*sin(2*theta);
		   Y += action[34]*cos(4*theta)+action[35]*sin(4*theta)-(action[30]+action[32]+action[34])+1.57;
		   ...
		   //add length
		   double cur_len = 0;
		   cur_len += pow(pose.position.x-pose.position.x,2);
		   cur_len += pow(pose.position.y-pose.position.y,2);
		   cur_len += pow(pose.position.z-pose.position.z,2);
		   cur_len = sqrt(cur_len);
		   cur_len += sqrt(pow(R,2)+pow(P,2)+pow(Y,2));
		   path_len += cur_len;
