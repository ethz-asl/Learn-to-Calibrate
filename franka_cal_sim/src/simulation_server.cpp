...
/*
This file is the simulaitionn server, functions:
*receive simulation request from the central client
*plan trajectory in the Gazebo according to the action of the agent using moveit
*return the trajectory length back to the central client
*/
...
namespace franka_cal_sim
{
	...
	bool simulation_server::model(franka_cal_sim::actionSrv::Request &req, franka_cal_sim::actionSrv::Response &res)
	{
		...
		//moveit group setup, must be done when the physics is not paused
		...
		//plan the trajectory with way points
		...
		//move the robot arm according to the plan
