...
bool simulation_server::model(franka_cal_sim::actionSrv::Request &req, franka_cal_sim::actionSrv::Response &res)
{
  ...
  for (int i = 0; i < num_of_step; i++)
  {
	...
	pose.position.x +=
		action[4] * cos(4 * theta) + action[5] * sin(4 * theta) - (action[0] + action[2] + action[4]) + 0.4;
	pose.position.y =
		action[6] * cos(theta) + action[7] * sin(theta) + action[8] * cos(2 * theta) + action[9] * sin(2 * theta);
	pose.position.y +=
		action[10] * cos(4 * theta) + action[11] * sin(4 * theta) - (action[6] + action[8] + action[10]) + 0.3;
	pose.position.z =
		action[12] * cos(theta) + action[13] * sin(theta) + action[14] * cos(2 * theta) + action[15] * sin(2 * theta);
	pose.position.z +=
		action[16] * cos(4 * theta) + action[17] * sin(4 * theta) - (action[12] + action[14] + action[16]) + 0.55;
