...
class ActorCritic:
    def __init__(self, sess):
        ...
        self.batch_size = 16
        self.explore_rate = 0.0002 #0.005  #0.003 #0.002
        self.num_trial = 1
        self.epsilon = .9
        ...
        self.target_state_input,self.target_actor_model = self.create_target_actor_model()
    ...
    def create_actor_model(self):
        # ========================================================================= #
        #hidden -> action
        state_input = Input(shape = (self.hidden_dim))
        h = Dense(500, activation='relu')(state_input)
        output = Dense(self.act_dim, activation='linear')(h)
        output_clip = tf.clip_by_value(output, -1, 1)
        model = Model([state_input], output_clip)
        adam  = Adam(lr=0.0001)
    ...
    def _train_actor(self, samples):
        ...
        predicted_actions = self.actor_model.predict([new_hiddens])
    ...
    def _train_critic(self, samples):
        ...
        new_b_states = self.target_hidden_model.predict([new_states, new_act_hists,new_hiddens])
        target_actions = self.target_actor_model.predict([new_b_states])
    ...
    def act(self, b_state):
        self.epsilon *= self.epsilon_decay
        rospy.loginfo(rospy.get_caller_id() + 'decay %s',self.epsilon)
        if np.random.random() < self.epsilon:
            ##Change to normal, random or normal is better?
            #0.005
            return np.random.normal(size=(36,))*self.explore_rate+self.actor_model.predict([b_state])*0.02
        return self.actor_model.predict([b_state])*0.02


    def deter_act(self, b_state):
        return self.actor_model.predict([b_state])*0.02
...
def simulation():
    ...
    for i in range(num_trials):
        ...
        for j in range(trial_len):
            ...
            action = actor_critic.act(new_hidden)
        ...
        if i % 5 == 0:
            ...
            for j in range(trial_len):
                ...
                #interact
                new_hidden = actor_critic.get_new_hidden(cur_state,last_act,hidden)
                action = actor_critic.deter_act(new_hidden)
                action = action.reshape((action_dim))
                cur_state = cur_state.reshape(obs_dim)
                new_state, reward, done = step(cur_state,action)
