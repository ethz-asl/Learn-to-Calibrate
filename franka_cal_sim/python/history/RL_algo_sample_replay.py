...
class ActorCritic:
    ...
    def create_actor_model(self):
        ...
        actor_rnn,state_h = GRU(32, return_state=True)(Concatenate()([obs_h1,act_h1]))
    ...
    def create_critic_model(self):
        ...
        mask_state_input = Masking(mask_value=0.)(state_input)
        mask_action_input = Masking(mask_value=0.)(act_hist_input)
        state_h1 = Dense(256, activation='relu',kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(mask_state_input)
        action_h1 = Dense(256, activation='relu',kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(mask_action_input)
        critic_rnn,state_h2 = GRU(32, return_state=True,kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(Concatenate()([state_h1,action_h1]))

        action_input = Input(shape=self.act_dim)
        action_h1    = Dense(500,kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(action_input)

        merged    = Concatenate()([state_h2, action_h1])
        merged_h1 = Dense(500, activation='relu',kernel_regularizer=tf.keras.regularizers.l2(self.reg_w))(merged)
...
def simulation():
    ...
    path = "/home/yunke/prl_proj/panda_ws/src/franka_cal_sim/python/replay_buffer.txt"
    #read replay buffer
    read_replay(path,actor_critic,obs_dim,action_dim)
