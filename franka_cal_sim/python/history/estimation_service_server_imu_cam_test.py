...
k_command = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 1 --dont-show-report"
k_command_total = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 30 --dont-show-report"
...
def compute_imu_entropy(imu_data):
    '''
    ---
    header:
    seq: 10750
...
def imu_calibration(num_step):
    ...
    imu_obs = H_Aopt*10000000
    imu_par_upd = state
    imu_progress = np.asarray(H_eig)*10000000
    #add camera err here
    imu_coverage = (orientation_x_en+orientation_w_en+orientation_y_en+orientation_z_en)*0.1

    return imu_par_upd, imu_progress, imu_obs, imu_coverage
...
def estimate_callback(req):
    ...
    if not req.reset == 1:
        ...
        cal_int = [best_mtx[0,0],best_mtx[1,1],best_mtx[0,2],best_mtx[1,2]]
        cam_err = np.linalg.norm(np.asarray(cam_int_truth)-np.asarray(cal_int))/100
        ...
        if cam_err<0.05:
            imu_par_upd,imu_progress,imu_obs,imu_coverage = imu_calibration(num_step)
        ...
        res.par_upd = np.concatenate([imu_par_upd,cam_par_upd,imu_progress,cam_progress])
        #add camera err here
        res.coverage = imu_coverage+cam_coverage
        rospy.loginfo(rospy.get_caller_id() + 'I get par_upd %s', res.par_upd)
        rospy.loginfo(rospy.get_caller_id() + 'I get cam obs %s',
                      cam_obs)
        rospy.loginfo(rospy.get_caller_id() + 'I get cam coverage %s',
                      cam_coverage)
        rospy.loginfo(rospy.get_caller_id() + 'I get imu obs %s',
                      imu_obs)
