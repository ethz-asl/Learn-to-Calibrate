...
class ActorCritic:
    ...
    def create_critic_model(self):
        ...
        critic_rnn,state_h2 = GRU(256, return_state=True)(Concatenate()([state_h1,action_h1]))
    ...
    def act(self, cur_state, cur_act_hist):
        ...
        if np.random.random() < self.epsilon:
            ...
            #0.005
            return np.random.normal(size=(36,))*self.explore_rate+self.actor_model.predict([cur_state,cur_act_hist])*0.03
    ...
    for i in range(num_trials):
        ...
        if i % 5 == 0:
            ...
            for j in range(trial_len):
                ...
                obs_seq = obs_seq.reshape((1, -1, obs_dim))
                act_seq = act_seq.reshape((1, -1, action_dim))
                action = actor_critic.actor_model.predict([obs_seq,act_seq])*0.03
                action = action.reshape((action_dim))
                cur_state = cur_state.reshape(obs_dim)
                new_state, reward, done = step(cur_state,action)
