...
k_command = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 1 --recover-covariance --dont-show-report"
k_command_total = "kalibr_calibrate_imu_camera --bag data.bag --cam camchain.yaml --imu imu.yaml --target checkerboard_7x6_10x10cm.yaml --max-iter 8 --recover-covariance --dont-show-report"
...
def estimate_callback(req):
    ...
    if not req.reset == 1:
        ...
        res.par_upd = np.concatenate([np.asarray(extrinsic).reshape((12,)),np.asarray(H_eig*100000000)])
        res.coverage = (orientation_x_en+orientation_w_en+orientation_y_en+orientation_z_en)*0.01
