...
def _get_corners(img, board, refine=True, checkerboard_flags=0):
    ...
    (ok, corners) = cv2.findChessboardCorners(
        ...
        flags=cv2.CALIB_CB_FAST_CHECK | checkerboard_flags) #cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FAST_CHECK |
    if not ok:
        return (ok, corners)
...
def estimate_callback(req):
    ...
    if not req.reset==1:
        ...
        reproj_err = 1
        try:
            best_mtx, ipts, opts, progress, reproj_err = camera_intrinsic_calibration2(req, image_data)
            rospy.loginfo(rospy.get_caller_id() + 'I get parameters %s',
                      best_mtx[0, 0])
        except:
            pass
