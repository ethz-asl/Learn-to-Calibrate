    ...
    for root, dirs, files in os.walk(kalibr_path):
        if 'results-imucam-data.txt' in files:
            ...
            for i in range(len(ext_lines)):
                ext_lines[i] = ext_lines[i].rstrip().strip("[]").strip(
                    " [ ").split()
...
def spawn_model(spawn_flag):
    ...
    if spawn_flag:
        ...
        #broacast new params
        tru_ext = np.asarray([
            -sample_ext[1], sample_ext[0], sample_ext[2], sample_ext[3],
            sample_ext[4], sample_ext[5]
        ])
        tru_ext = (
            tru_ext.astype(float) +
            np.asarray([0.06, 0.0, -0.1 - 0.0584, 0, 0, 1.5708])).tolist()
        rospy.set_param("/rl_client/imu_cam_ground_truth", tru_ext)
        rospy.set_param("/rl_client/cam_ground_truth", intrinsic)

        rospy.loginfo(rospy.get_caller_id() + 'I get if_load %s', if_load)
