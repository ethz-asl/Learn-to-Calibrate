...
def spawn_model(spawn_flag):
    ...
    unload_controller_srv = rospy.ServiceProxy('/controller_manager/unload_controller',UnloadController)
    reload_controller_srv = rospy.ServiceProxy('/controller_manager/reload_controller_libraries',ReloadControllerLibraries)
    ...
    if spawn_flag:
        ...
        #broacast new params
        tru_ext = np.asarray([-sample_ext[1],sample_ext[0],sample_ext[2],sample_ext[3],sample_ext[4],sample_ext[5]])
        tru_ext = (tru_ext.astype(float)+np.asarray([0.06,0.0,-0.1-0.0584,0,0,1.5708])).tolist()
        rospy.set_param("/rl_client/imu_cam_ground_truth",tru_ext)
        rospy.set_param("/rl_client/cam_ground_truth",intrinsic)

        rospy.loginfo(rospy.get_caller_id() + 'I get if_load %s',
                      if_load)
...
def estimate_callback(req):
    ...
    if not req.reset == 1:
        ...
        rospy.loginfo(rospy.get_caller_id() + 'I get total entropy %s', entropy)
        rospy.loginfo(rospy.get_caller_id() + 'I get reproj err %s', reproj_err)
