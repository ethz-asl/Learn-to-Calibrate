...
class ActorCritic:
    ...
    def plan(self, cur_obs_hist, cur_act_hist,step_num):
        #given current status, plan a traj in step num using learned model
        tmp_obs_hist = copy.copy(cur_obs_hist)
        tmp_act_hist = copy.copy(cur_act_hist)
        total_reward = 0
        final_action = np.zeros(36)
        for step in range(step_num):
            action = np.random.uniform(-0.02,0.02,36)
            if step==0:
            ...
            obs_seq = obs_seq.reshape((1, -1, self.obs_dim))
            act_seq = act_seq.reshape((1, -1, self.act_dim))
            pred_obs = self.trans_model.predict([obs_seq,act_seq,action])
            pred_reward = self.reward_model.predict([obs_seq,act_seq,action])
    ...
    def act(self, cur_obs_hist, cur_act_hist, step_num, sample_num, israndom):
        #random action
        if israndom:
        ...
        for i in range(sample_num):
            total_reward, action = self.plan(cur_obs_hist,cur_act_hist,step_num)
...
def simulation():
    ...
    for i in range(num_trials):
        ...
        for j in range(trial_len):
            ...
            action = actor_critic.act(obs_list,act_list,step_num,20,israndom)
        ...
        if i % 5 == 0:
            ...
            for j in range(trial_len):
                ...
                act_seq = act_seq.reshape((1, -1, action_dim))
                step_num = trial_len-j
                action = actor_critic.act(obs_list,act_list,step_num,20,0)
                action = action.reshape((action_dim))
                cur_state = cur_state.reshape(obs_dim)
                new_state, reward, done = step(cur_state,action)
