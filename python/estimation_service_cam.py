...
def cal_fromcorners(req, good, camera_model, size):
    ...
    criteria_cal = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 50,
                    0.0001)
...
def camera_intrinsic_calibration2(req, image_data, only_progress=0):
    ...
    print(best_mtx)
    '''
    return mtx, ipts, opts, progress, state_progress, reproj_err
...
def estimate_callback(req):
    ...
    if not req.reset==1:
        ...
        done = state_progress[0] < 0.4 and state_progress[1] < 0.4 
        done = done and state_progress[2] < 0.4 and state_progress[3] > 0.6
        done = done and state_progress[4] > 0.6 and state_progress[5] > 0.6
        done = done and state_progress[6] > 0.6 and state_progress[7] < 0.4
        ...
        reproj_err = 1
        if done or num_step ==cal_num_step:
            ...
            abs_cal_err = np.linalg.norm(cal_param-ground_truth)

            #respawn the model
            if_change_model = rospy.get_param("/rl_client/if_change_sensors")

            
        if (num_step >=cal_num_step or done) and if_change_model:
        ...
        if len(good_corners)>0:
            ...
            res.cal_err = 1.
            if done or num_step ==cal_num_step:
                res.cal_err = abs_cal_err/np.linalg.norm(ground_truth)
                if if_change_model:
