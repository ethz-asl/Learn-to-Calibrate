...
class ActorCritic:
    def __init__(self, sess):
        ...
        self.critic_learning_rate = 5e-5  #1e-4 #5e-5 #5e-5
        self.actor_learning_rate = 5e-5   #1e-4 #5e-5 #5e-5
        self.batch_size = 32
        self.explore_rate = 0.2 #0.005  #0.003 #0.002
        self.num_trial = 1
        self.epsilon = .9
        ...
        self.actor_act_hist_input, self.actor_model = self.create_actor_model()
        ...
        self.critic_act_hist_input, self.critic_action_input, \
    ...
    def create_actor_model(self):
        ...
        actor_rnn,state_h = GRU(256, return_state=True)(act_h1)
        h2 = Dense(500, activation='relu')(state_h)
        output = Dense(self.act_dim, activation='tanh')(h2)

        model = Model([act_hist_input], output)
        adam  = Adam(lr=0.0001)
        model.compile(loss="mse", optimizer=adam)
        return act_hist_input, model

    def create_critic_model(self):
        act_hist_input = Input(shape=(None,self.act_dim))
        mask_action_input = Masking(mask_value=0.)(act_hist_input)
        action_h1 = Dense(256, activation='relu')(mask_action_input)
        critic_rnn,state_h2 = GRU(256, return_state=True)(action_h1)

        action_input = Input(shape=self.act_dim)
        action_h2    = Dense(500)(action_input)
    ...
    def _train_actor(self, samples):
        ...
        cur_act_hists, actions, rewards, new_act_hists, _ =  stack_samples(samples)
        predicted_actions = self.actor_model.predict([cur_act_hists])
    ...
    def _train_critic(self, samples):
        ...
        target_actions = self.target_actor_model.predict([new_act_hists])
        future_rewards = self.target_critic_model.predict([new_act_hists,target_actions])
        ...
        evaluation = self.critic_model.fit([cur_act_hists, actions], rewards, verbose=0)
    ...
    def act(self, cur_act_hist):
        self.epsilon *= self.epsilon_decay
        rospy.loginfo(rospy.get_caller_id() + 'decay %s',self.epsilon)
        if np.random.random() < self.epsilon:
            ##Change to normal, random or normal is better?
            #0.005
            return np.random.normal(size=(5,))*self.explore_rate+self.actor_model.predict([cur_act_hist])*0.5
        return self.actor_model.predict([cur_act_hist])*0.5
...
def simulation():
    ...
    for i in range(num_trials):
        ...
        for j in range(trial_len):
            ...
            action = actor_critic.act(act_seq)
            ...
            rospy.loginfo(rospy.get_caller_id() + 'act_shape %s',act_seq.shape)
        ...
        if i % 5 == 0:
            ...
            obs_list = []
            act_list = []
            act_list.append(normalize(0.00*np.ones((1,action_dim))))
            for j in range(trial_len):
                #env.render()
                print("test:" + str(i))
                print("step:" + str(j))
                ...
                act_seq = np.asarray(act_list)
                act_seq = act_seq.reshape((1, -1, action_dim))
                action = actor_critic.actor_model.predict([act_seq])*0.5
                action = action.reshape((action_dim))
                last=0
                if j == (trial_len - 1):
                    last = 1
                ...
                if done:
                    ...
                    fig1.savefig('/home/yunke/prl_proj/panda_ws/src/franka_cal_sim_single/python/test_reward.png')
