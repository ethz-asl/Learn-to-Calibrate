...
class ValueNetwork(nn.Module):
    def __init__(self, set_state_dim, state_dim, hidden_dim, init_w=3e-3):
        ...
        self.linear2 = nn.Linear(3*hidden_dim, hidden_dim)
    ...
    def forward(self, set_state, nom_state):
        ...
        pool1 = nn.MaxPool1d(set_x.size(-1))(set_x)
        flat1 = nn.Flatten(1)(pool1)

        set_x = torch.cat((set_x, pool1.repeat(1, 1, set_x.size(-1))), dim=-2)
        set_x = F.sigmoid(self.conv4(set_x))
        set_x = F.sigmoid(self.conv5(set_x))
        set_x = F.sigmoid(self.conv6(set_x))
        pool2 = nn.MaxPool1d(set_x.size(-1))(set_x)
        flat2 = nn.Flatten(1)(pool2)
        # flat1 = nn.Flatten(1)(ave_pool)
        # min_pool = torch.min(set_x,dim=-1)

        x = F.relu(self.linear1(nom_state))
        x = F.relu(self.linear2(torch.cat((x, flat1, flat2), dim=-1)))
        x = self.linear3(x)
        x = self.linear4(x)
        return x, flat1, flat2
...
class SoftQNetwork(nn.Module):
    def __init__(self, set_num_inputs, num_inputs, num_actions, hidden_size, init_w=3e-3):
        ...
        self.linear2 = nn.Linear(3*hidden_size, hidden_size)
...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, set_state, nom_state):
        ...
        return mean, log_std, flat1, flat2
    
    def evaluate(self, set_state, nom_state, epsilon=1e-6):
        mean, log_std, flat1, flat2 = self.forward(set_state, nom_state)
        std = log_std.exp()
        ...
        return action, log_prob, z, mean, log_std, flat1, flat2
    ...
    def get_action(self, set_state, nom_state):
        ...
        mean, log_std, _, _ = self.forward(set_state, nom_state)
        std = log_std.exp()
...
def soft_q_update(batch_size, 
    ...
    expected_q_value, f1_q, f2_q = soft_q_net(set_state, nom_state, action)
    expected_value, f1_v, f2_v   = value_net(set_state, nom_state)
    new_action, log_prob, z, mean, log_std, f1_p, f2_p = policy_net.evaluate(set_state, nom_state)

    dot_q = torch.bmm(f1_q.view(-1, 1, hidden_dim), f2_q.view(-1, hidden_dim, 1))
    dot_v = torch.bmm(f1_v.view(-1, 1, hidden_dim), f2_v.view(-1, hidden_dim, 1))
    dot_p = torch.bmm(f1_p.view(-1, 1, hidden_dim), f2_p.view(-1, hidden_dim, 1))


    target_value,_, _ = target_value_net(next_set_state, next_nom_state)
    next_q_value = reward + (1 - done) * gamma * target_value
    q_value_loss = soft_q_criterion(expected_q_value, next_q_value.detach()) + 0.1 * torch.mean(dot_q)

    expected_new_q_value, _, _ = soft_q_net(set_state, nom_state, new_action)
    next_value = expected_new_q_value - log_prob
    value_loss = value_criterion(expected_value, next_value.detach()) + 0.1 * torch.mean(dot_v)
    ...
    policy_loss += mean_loss + std_loss + z_loss + 0.1 * torch.mean(dot_p)
